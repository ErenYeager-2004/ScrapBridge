...
- [Misc] Updated .gitignore to exclude local project documentation: context.md, GUIDE.md, mail.md, and ScrapBridge_Implementation_Plan.docx.
...
- [Bug Fix] Fixed three linter errors in `NotificationBell.jsx` and `useFetch.js`:
  1. **"Cannot access variable before it is declared"** (`NotificationBell.jsx`): Moved `fetchNotifications` and `markAllRead` function declarations above the `useEffect` hooks.
  2. **"Calling setState synchronously within an effect"** (`NotificationBell.jsx`): Replaced the standalone `fetchNotifications` async function + `useEffect` call pattern with a named async function (`loadNotifications`) defined *inside* the `useEffect` body. This makes the async boundary explicit and satisfies React's effect rules.
  3. **"setRefetchTick is assigned a value but never used"** (`NotificationBell.jsx`): Removed the unused `refetchTick`/`setRefetchTick` state that was added unnecessarily, simplified the fetch effect back to a `[]` dependency array.
- [Bug Fix] Fixed **"Expected the dependency list for useCallback to be an array literal"** (`useFetch.js`): Removed the dynamic `deps` parameter from `useFetch` — passing a runtime variable to `useCallback` defeats static analysis. The hook now uses `[fetcherFn]` as the literal dependency array. Callers needing refetch-on-value-change should wrap their fetcher in `useCallback` at the call-site.
